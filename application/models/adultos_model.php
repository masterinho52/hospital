...
class Adultos_model extends CI_Model {
	...
	public function get_all_between($desde, $hasta) {
		$consulta = $this->db->join('tbl_representante', 'tbl_representante.id_representante=tbl_adulto.id_representante')
					->where('estatus_adulto',1)
					->where('fechai_adulto BETWEEN "'.$desde.'" AND "'.$hasta.'"')
					->order_by('apellido_adulto asc, nombre_adulto asc')
					->get('tbl_adulto');
		if ($consulta->num_rows()) {
			$data = $consulta->result();
		} else {
	...
	public function estatus_uno($id_adulto) {
		...
		$data_adulto = array ('estatus_adulto' => 1);
		$data = $this->db->where('id_adulto', $id_adulto)->update('tbl_adulto', $data_adulto);
		if ($data) { return TRUE; } else { return FALSE; }
	}
