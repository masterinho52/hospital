...
<html lang="es">
	<head>
  	...
  	<body>
		<div id="container">
			...
			<div id="body">
				...
				<p>
					<h3 class="centrado">Impresión de Informe Social</h3>
					<form action="<?=base_url()?>impresiones/social_print" id="formulario" method="post" name="formulario" target="_blank">
					<table align="center" border="0" cellpadding="0" cellspacing="2" class="texto" width="100%">
						<tr height="20"><td colspan="3"></td></tr>

						<tr>
							<td width="22%"></td>
							<td width="18%">Inf. Social del Adulto:</td>
							<td width="60%">
								<select name="id_social" id="id_social" class="combo">
			                      	<option value=""></option>
			                      	<?php if($soc) : ?>
			                    		<?php foreach($soc as $row) : ?>
			                      			<option value="<?=$row->id_social;?>"><?=$row->apellido_adulto; ?> <?=$row->nombre_adulto; ?> - <?=$row->cedula_adulto; ?></option>
			                        	<?php endforeach; ?>
			                      	<?php endif; ?>
			                    </select>
							</td>
  	...
  	<script type="text/javascript">
		...
		frmvalidator.addValidation("id_social","req","El campo del Informe Social esta vacio, ¡Debe seleccionar los Datos!");
    </script>
</html>